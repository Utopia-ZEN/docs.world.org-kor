...
def run(cfg: Config) -> Dict[str, object]:
    ...
    try:
        ...
        errors.append(f"[SITEMAP_ERR] {cfg.sitemap_url}: {exc}")
        print(errors[-1])
    ...
    for url in urls:
        try:
            ...
            if "429" in msg or "Too Many Requests" in msg:
                rate_limit_errors += 1
                # Slow down aggressively when the API reports rate limiting.
                time.sleep(min(60, 10 * rate_limit_errors))
                if rate_limit_errors >= 3:
                    errors.append("[ABORT] Too many consecutive 429 errors. Stopping this run.")
                    print(errors[-1])
                    break
    build_index(cfg.output_dir, processed)
